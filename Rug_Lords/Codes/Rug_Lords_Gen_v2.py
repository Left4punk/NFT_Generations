...
def Get_Rarities ():
    ...
    BackGrounds_Rarity = [4,4,12,4,4,4,12,12,4,4,4,4,4,4,4,12,4,4,12]
    Base_Rarity = [21,22,5,3,22,23,4]
    Torso_Rarity=[4,4,6,6,6,6,4,3,3,6,4,4,4,4,6,6,6,6,6,3,3]
    FacialHair_Rarity=[8,8,8,8,7,50,8,3]
    Mouth_Rarity=[10,5,80,5]
    Facewear_Rarity = [3,3,2,2,3,1,5,55,3,6,8,4]
    Head_Rarity = [4,4,4,4,4,4,3,3,3,2,4,3,3,3,3,3,3,2,3,2,4,2,3,3,3,3,4,4,4,4,2,2,1,1]
...
def Get_NFT_Hands_Expression(Base):
    ...
    if (Base in ['White','Tan','Brown','Black']):
        ...
        Hand_Rarity = [3,4,4,4,4,4,50,1.7,4,5,5,5,3,3,3,5,4,5,3,3,3]
    else:
        Hand_Rarity = [3,4,4,4,4,4,50,1.7,4,3,3,3,5,4,5,3,3,3]
...
def FacialHair_Conflicts (NFT_FacialHair, FacialHair, FacialHair_Rarity, NFT_Head):
    ...
    if NFT_Head in ['Flow hair brown.png', 'Mullet brown.png', 'Short hair brown.png', 'Side part hair brown.png', 'Headband.png']:
        ...
        conflict_FacialHair = ['Duck tail black.png', 'Full beard black.png','Stubble black.png']
...
def update_traits_count(category, trait):

    if trait in traits_count[category]:
        traits_count[category][trait] += 1
    else:
        traits_count[category][trait] = 1

def calculate_and_display_trait_percentages(total_nfts, output_file_path):

    with open(output_file_path, 'w') as file:
        for category, traits in traits_count.items():
            file.write(f"\nCategory: {category}\n")
            for trait, count in traits.items():
                percentage = (count / total_nfts) * 100
                count = int(count)
                file.write(f"{trait}: {percentage:.2f}%, {count}\n")
...
if __name__ == '__main__':
    ...
    while (i<collection_number):
        ...
        if (NFT_Mouth != 'None.png'):
            ...
            conflict_background = ['Binary code.png','Binary purple.png','Night sky.png','Mosque.png', 'Sunset.png','Beach.png','Cyan.png','Pyramid.png']
            conflict_head = ['Grey & Red turban.png']
        ...
        if ((NFT_Hand == 'Treasure.png' or NFT_Hand == 'Weed plant.png') and NFT_Mouth == 'Pipe.png'):
        ...
        if (NFT_Base == 'Cyborg.png' and NFT_Facewear in ['Laser eyes cyan.png','Laser eyes green.png']):
        ...
        ##making sure we only generate 100 OR runes
        if NFT_Hand == 'OR rune.png':

            OR_runes_number += 1
        
        if OR_runes_number>100 and NFT_Hand == 'OR rune.png':
            print ('100 Runes Done')
            continue 
        ...
        ## updating trait counter
        update_traits_count('Background', NFT_BackGrounds.split('.')[0])
        update_traits_count('Base', NFT_Base.split('.')[0])
        update_traits_count('Expression', NFT_Expression.split('.')[0])
        update_traits_count('Torso', NFT_Torso.split('.')[0])
        update_traits_count('Facial Hair', NFT_FacialHair.split('.')[0])
        update_traits_count('Mouth', NFT_Mouth.split('.')[0])
        update_traits_count('Facewear', NFT_Facewear.split('.')[0])
        update_traits_count('Head', NFT_Head.split('.')[0])
        update_traits_count('Whole Body', NFT_WholeBody.split('.')[0])
        update_traits_count('Hand', NFT_Hand.split('.')[0])
        ## updating trait counter
    ...
    calculate_and_display_trait_percentages(i-1, 'trait_percentages.txt')