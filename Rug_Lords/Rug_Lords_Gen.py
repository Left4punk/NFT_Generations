...
def Get_Layers ():
    ...
    BackGrounds = [f for f in listdir(f"{input_path}\Rug_Lords_Traits/0 - Background") if isfile(join(f"{input_path}\Rug_Lords_Traits/0 - Background", f))]
    Base = [f for f in listdir(f"{input_path}\Rug_Lords_Traits/1 - Base") if isfile(join(f"{input_path}\Rug_Lords_Traits/1 - Base", f))]
    Eyes = [f for f in listdir(f"{input_path}\Rug_Lords_Traits/2 - Eyes") if isfile(join(f"{input_path}\Rug_Lords_Traits/2 - Eyes", f))]
    Torso = [f for f in listdir(f"{input_path}\Rug_Lords_Traits/3 - Torso") if isfile(join(f"{input_path}\Rug_Lords_Traits/3 - Torso", f))]
    Mouth = [f for f in listdir(f"{input_path}\Rug_Lords_Traits/4 - Mouth") if isfile(join(f"{input_path}\Rug_Lords_Traits/4 - Mouth", f))]
    FacialHair = [f for f in listdir(f"{input_path}\Rug_Lords_Traits/5 - Facial Hair") if isfile(join(f"{input_path}\Rug_Lords_Traits/5 - Facial Hair", f))]
    Accesories = [f for f in listdir(f"{input_path}\Rug_Lords_Traits/6 - Accesories") if isfile(join(f"{input_path}\Rug_Lords_Traits/6 - Accesories", f))]
    Head = [f for f in listdir(f"{input_path}\Rug_Lords_Traits/7 - Head") if isfile(join(f"{input_path}\Rug_Lords_Traits/7 - Head", f))]
    Facewear = [f for f in listdir(f"{input_path}\Rug_Lords_Traits/8 - Facewear") if isfile(join(f"{input_path}\Rug_Lords_Traits/8 - Facewear", f))]

    return  BackGrounds, Base, Eyes, Torso, Mouth, FacialHair, Accesories, Head, Facewear

def Get_Rarities ():
    
    BackGrounds_Rarity = [33,33,33]
    Base_Rarity = [100]
    Eyes_Rarity = [10,10,10,50,10,10,10]
    Torso_Rarity=[10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10]
    Mouth_Rarity=[10,10,10,50,10,10,10]
    FacialHair_Rarity=[10,10,10,10,10,10]
    Accesories_Rarity = [33,33,33]
    Head_Rarity = [10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10,10]
    Facewear_Rarity = [10,10,10,10,10,10,10,10,10]
...
def get_Images (NFT_BackGrounds, NFT_Base, NFT_Eyes, NFT_Torso, NFT_Mouth, NFT_FacialHair, NFT_Accesories, NFT_Head, NFT_Facewear):
    ...
    Image_BackGrounds = f'{input_path}\Rug_Lords_Traits/0 - Background/'+NFT_BackGrounds
    Image_Base = f'{input_path}\Rug_Lords_Traits/1 - Base/'+NFT_Base
    Image_Eyes = f'{input_path}\Rug_Lords_Traits/2 - Eyes/'+NFT_Eyes
    Image_Torso = f'{input_path}\Rug_Lords_Traits/3 - Torso/'+NFT_Torso
    Image_Mouth = f'{input_path}\Rug_Lords_Traits/4 - Mouth/'+NFT_Mouth
    Image_FacialHair = f'{input_path}\Rug_Lords_Traits/5 - Facial Hair/'+NFT_FacialHair
    Image_Accesories = f'{input_path}\Rug_Lords_Traits/6 - Accesories/'+NFT_Accesories
    Image_Head = f'{input_path}\Rug_Lords_Traits/7 - Head/'+NFT_Head
    Image_Facewear = f'{input_path}\Rug_Lords_Traits/8 - Facewear/'+NFT_Facewear
...
if __name__ == '__main__':
    ...
    input_path = rf'C:\Users\pcostasv\OneDrive - PUIG\Escritorio\Personal\Python Scripts\Rug_Lords_Algo\Images'
    output_path_nft = rf'C:\Users\pcostasv\OneDrive - PUIG\Escritorio\Personal\Python Scripts\Rug_Lords_Algo\Generations'
    output_path_json = rf'C:\Users\pcostasv\OneDrive - PUIG\Escritorio\Personal\Python Scripts\Rug_Lords_Algo\Metadata'
    ...
    ##import images
        ...
        final.save (f'{output_path_nft}\Rug_Lord #{i}.png')
        ...
        json_out = open(rf'{output_path_json}\Rug_Lord #{i}.json','w')
